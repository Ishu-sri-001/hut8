...
// Memoized HTML markers component
const HtmlMarkers = memo(() => (
  <>
    {/* windmills */}
...
const Model = memo(({ url, textures = {}, initialOpacity = 1, rotation, opacity, position, scale = 1, showHtmlMarkers = false }) => {
  const group = useRef();
  ...
  const loadedTextureArray = textureUrls.length > 0 ? useTexture(textureUrls) : [];
  
  const loadedTextures = useMemo(() => {
    const result = {};
    const textureKeys = Object.keys(textures);
    textureKeys.forEach((key, index) => {
      result[key] = Array.isArray(loadedTextureArray) ? loadedTextureArray[index] : loadedTextureArray;
    });
    return result;
  }, [textureUrls.length]);
  ...
  useEffect(() => {
    if (group.current && !materialsInitialized.current) {
      group.current.traverse((child) => {
        if (child.isMesh) {
          ...
          child.material.depthWrite = initialOpacity >= 0.99;
          child.renderOrder = 1;
  ...
  }, [initialOpacity, loadedTextures]);

  useFrame(() => {
    if (group.current) {
      const rot = rotation.current;
      const pos = position.current;
      const opa = opacity.current;
      ...
      group.current.traverse((child) => {
        if (child.isMesh && child.material) {
          if (Math.abs(child.material.opacity - opa) > 0.001) {
            child.material.opacity = opa;
            child.material.depthWrite = opa >= 0.99;
          }
        }
      });
    }
  ...
  return (
    <group ref={group}>
      ...
      {showHtmlMarkers && opacity.current > 0.1 && <HtmlMarkers />}
    </group>
  );
});
...
function Scene() {
  ...
  return (
    <>
      ...
      <directionalLight position={[10, 10, 5]} intensity={12.0} castShadow={false} />
      <directionalLight position={[-10, -10, -5]} intensity={12.5} castShadow={false} />
